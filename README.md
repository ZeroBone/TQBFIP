...
in the `src` directory. Here, by replacing `<seed>` with some integer, it is possible to adjust the verifier's random number generator seed. This is useful when we want to execute the protocol multiple times without having the numbers the verifier chooses at random change every time. In case this parameter is omitted, a default fixed seed will be used.
...
The entire animation can be viewed [on YouTube](https://youtu.be/gSWT3DOiQtk).
